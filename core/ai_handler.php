...
function generateQuestionWithGroq($topic, $previousQuestions = []) {
    ...
    $prompt = <<<PROMPT
Создай сложный и забавный игровой контент на русском языке на тему "$topic". $excludeText

Ты должен случайным образом выбрать один из двух форматов:
Формат 1: Необычный, сложный или смешной вопрос с множественным выбором.
Формат 2: Утверждение или предложение с пропущенным ключевым словом, где пропуск в предложении обозначен как `_____` (ровно 5 символов подчеркивания). Правильный ответ — это пропущенное слово в нужной грамматической форме.

Требования к качеству и стилю контента (ВАЖНО!):
- Вопрос или предложение должны быть СТРОГО связаны с заданной темой "$topic". Категорически запрещено переключаться на посторонние темы, даже если тема кажется сложной!
- Вопрос или предложение должны быть сложными, забавными, на грани легкого абсурда, здорового бреда или парадокса.
- ИЗБЕГАЙ банальных обобщений и скучных вопросов из школьной программы (например, "известен своими видео/мемами/книгами", "родился в таком-то году" или простейшие научные факты). 
- Ищи КОНКРЕТНЫЕ малоизвестные курьезные факты, нелепые случаи из биографии, странные привычки, абсурдные законы, забавные цитаты или курьезные научные феномены.
- Примеры для вдохновения:
  * Формат 1: "Зачем в 1913 году почтовая служба США временно запретила отправлять детей почтой?" -> "родители отправляли детей посылками к бабушкам, потому что это было дешевле билета"
  * Формат 2: "В 18 веке в Англии богатые люди нанимали специальных людей, которые _____ на их званых обедах." -> "молчали"
- Правильный ответ должен быть абсолютно точным, исторически или научно подтвержденным (или логически вытекать из контекста) и коротким (1-3 слова).
- Для формата с пропуском (`_____`): все варианты ответов (правильный и 10 неправильных в массиве fakes) должны грамматически идеально подходить на место пропуска (одинаковый падеж, род, число, часть речи), чтобы игрок не мог угадать ответ чисто по грамматической структуре.
- Неправильные ответы (fakes) должны быть убедительными, но при этом забавными, ироничными или слегка абсурдными альтернативами (в стиле игры Jackbox Party Pack / Fibbage), в которые весело поверить. Избегай скучных и очевидных вариантов.
- РОВНО 10 неправильных ответов в массиве fakes.
- Все тексты пиши строго на русском языке.

Формат ответа (выдай ТОЛЬКО валидный JSON, без какого-либо дополнительного текста, markdown-разметки или объяснений):
{
  "question": "текст вопроса или предложение с пропуском (например: 'В 19 веке в Англии _____ использовались вместо грелок.')",
  "correct": "правильный ответ (или слово для пропуска)",
  "fakes": ["вариант 1", "вариант 2", "вариант 3", "вариант 4", "вариант 5", "вариант 6", "вариант 7", "вариант 8", "вариант 9", "вариант 10"]
}
PROMPT;
