...
## 5. Agent Workspace & Artifact Guidelines
- **.agents/rules/**: Agents MUST read relevant domain rules (e.g., `backend.md`, `frontend.md`, `testing.md`) before starting implementation.
- **scratch/**: Use this directory for temporary scripts, debugging, or test files. Do not commit these to Git.
- **.artifacts/**: Store long-lived context, Architectural Decision Records (ADRs), and session handover documents (e.g., `handoff_context.md`) here.
- **repomix.config.json**: Manages context packing. Modify only if new major directories are added to the monorepo.
