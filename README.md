...
## 🛠️ Инструменты разработчика (MCP)
В проекте Quete настроена поддержка **Model Context Protocol (MCP)**, которая позволяет ИИ-ассистентам безопасно взаимодействовать с вашей локальной базой данных, историей Git, браузером Puppeteer и удаленным сервером по SSH.

Подробные инструкции по работе с MCP-серверами, примеры запросов и правила конфигурации находятся в файле **[USER_INSTR.md](file:///c:/OSPanel/domains/quete/pre-alpha0.1.3-antigravity-git/USER_INSTR.md)**.
